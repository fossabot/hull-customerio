/* @flow */
import type { TCustomerIoCustomer, TBusinessValidationResult, IValidationUtilOptions } from "../types";
...
class ValidationUtil {
  ...
  constructor(options: IValidationUtilOptions) {
    ...
    this.maxAttributeValueLength = _.get(options, "maxAttributeValueLength", 1000);
    this.maxIdentifierValueLength = _.get(options, "maxIdentifierValueLength", 150);
  }
  ...
  validateCustomer(customer: TCustomerIoCustomer): TBusinessValidationResult {
    ...
    if (this.getBytes(id) > this.maxIdentifierValueLength) {
      ...
      validationResult.validationErrors.push(SHARED_MESSAGES.ERROR_VALIDATION_MAXLENGTHID);
    }
    ...
    _.forIn(attributes, (val, key) => {
      if (this.getBytes(key) > this.maxAttributeNameLength) {
        validationResult.isValid = false;
        validationResult.validationErrors.push(`${SHARED_MESSAGES.ERROR_VALIDATION_MAXLENGTHNAME} '${key}'`);
      }
      ...
      if (_.isObject(val)) {
        ...
        if (this.getBytes(strVal) > this.maxAttributeValueLength) {
          validationResult.isValid = false;
          validationResult.validationErrors.push(`${SHARED_MESSAGES.ERROR_VALIDATION_MAXLENGTHVALUE} '${strVal}' for attribute '${key}'`);
        }
      } else if (this.getBytes(val) > this.maxAttributeValueLength) {
        validationResult.isValid = false;
        validationResult.validationErrors.push(`${SHARED_MESSAGES.ERROR_VALIDATION_MAXLENGTHVALUE} '${val}' for attribute '${key}'`);
      }
    });
