...
describe("FilterUtil", () => {
  ...
  describe("getNumberFromContext", () => {
    test("should skip events that are not whitelisted and insert the remaining ones", () => {
      ...
      expect(getNumberFromContext(context, "ship.private_settings.max_attribute_name_length", 99)).toEqual(1000);
      expect(getNumberFromContext(context, "ship.private_settings.max_attribute_value_length", 98)).toEqual(100);
      expect(getNumberFromContext(context, "ship.private_settings.max_identifier_name_length", 97)).toEqual(97);
    });
  });
});
